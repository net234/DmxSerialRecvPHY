...
void loop() {
  ...
  if (lastPacket < 5000) {
    ...
    int aValue = 0;
    aValue = 255-DMXSerial.read(startChannel);
    aValue = aValue * 220 /255;
    aValue += 35;


    analogWrite(RedPin, aValue );
    analogWrite(GreenPin, 255-DMXSerial.read(startChannel + 1) );
    analogWrite(BluePin, 255-DMXSerial.read(startChannel + 2) );
